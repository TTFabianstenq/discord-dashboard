...
export const useRelay = create<RelayState>((set, get) => ({
  ...
  connectLive: async (rawToken) => {
    ...
    try {
      ...
      if (!bot.bot) {
        throw new Error("This token belongs to a user account. BotDeck only accepts bot tokens.");
      }
  ...
  loadMessages: async (channelId, force = false) => {
    ...
    try {
      ...
      set((s) => ({
        messages: { ...s.messages, [channelId]: ordered },
        loading: force ? s.loading : false,
      }));
    } catch (err) {
      if (force) return;
      set({
