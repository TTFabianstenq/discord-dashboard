...
import { destroyGateway, ensureGateway, type PresencePayload } from "./gateway";
...
export const useRelay = create<RelayState>((set, get) => ({
  ...
  loadGuild: async (guildId) => {
    ...
    if (mode === "demo") {
      ...
      const first = list.find((c) => isTextLike(c.type));
      const v = get().view;
      if (first && v.t === "guild" && v.id === guildId && !v.channelId) {
        set({ view: { ...v, channelId: first.id } });
      }
      return;
    }
    ...
    try {
      ...
      set((s) => {
        const shouldSelect =
          !!first && s.view.t === "guild" && s.view.id === guildId && !s.view.channelId;
        return {
  ...
  deleteRole: async (guildId, roleId) => {
    ...
    if (mode === "demo") {
      set((s) => ({
        roles: { ...s.roles, [guildId]: (s.roles[guildId] ?? []).filter((r) => r.id !== roleId) },
      }));
      return;
    }
    if (!token) throw new Error("Not connected.");
  ...
  createWebhook: async (channelId, name, avatar) => {
    ...
    if (!token) throw new Error("Not connected.");
    return await live<DiscordWebhook>(token, "POST", `/channels/${channelId}/webhooks`, {
      name,
  ...
  createCommand: async (data, guildId) => {
    ...
    if (mode === "demo" || !token || !application) throw new Error("Not available.");
    const path = guildId
      ? `/applications/${application.id}/guilds/${guildId}/commands`
      : `/applications/${application.id}/commands`;
    ...
    set((s) => {
      ...
      return { commands: { ...s.commands, global: [...s.commands.global, created] } };
    });
  },
  ...
  deleteCommand: async (commandId, guildId) => {
    ...
    set((s) => {
      ...
      return {
        commands: {
          ...s.commands,
          global: s.commands.global.filter((c) => c.id !== commandId),
        },
      };
    });
  },
