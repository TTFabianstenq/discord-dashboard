...
export const useRelay = create<RelayState>((set, get) => ({
  ...
  editMessage: async (channelId, messageId, content) => {
      ...
      set((s) => ({
        messages: {
          ...
          [channelId]: (s.messages[channelId] ?? []).map((m) =>
            m.id === messageId ? { ...m, content, edited_timestamp: new Date().toISOString() } : m,
          ),
        },
      }));
      return;
    ...
    if (!token) throw new Error("Not connected.");
    const updated = await live<DiscordMessage>(token, "PATCH", `/channels/${channelId}/messages/${messageId}`, {
      content,
    });
    set((s) => ({
      messages: {
