...
type UploadFile = { filename: string; contentType: string; dataBase64: string };
...
type RelayState = {
  ...
  gatewayDebug: GatewayDebug | null;
  view: AppView;
  loading: boolean;
  connecting: boolean;
  error: string | null;
  ...
  sendMessage: (
    ...
    content: string,
    embeds?: DiscordEmbed[],
    opts?: { messageReferenceId?: string; files?: UploadFile[] },
  ) => Promise<void>;
...
export const useRelay = create<RelayState>((set, get) => ({
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        const gw = ensureGateway(token, {
          ...
          onDebug: (debug) => useRelay.setState({ gatewayDebug: debug }),
  ...
  sendMessage: async (channelId, content, embeds, opts) => {
    ...
    const body: {
      ...
      allowed_mentions?: ReturnType<typeof buildAllowedMentions>;
    } = {};
    if (normalized.trim()) {
    ...
    if (!body.content && !body.embeds?.length && !(files && files.length)) {
      throw new Error("Write a message, add an embed, or attach an image.");
    }
    if (mode === "demo" && bot) {
  ...
  createWebhook: async (channelId, name, avatar) => {
    const { mode, token } = get();
    if (mode === "demo") throw new Error("Demo");
    if (!token) throw new Error("Not connected.");
