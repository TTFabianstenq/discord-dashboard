...
import { AtSign, Eraser, ImageIcon, MoreHorizontal, Pencil, Pin, RefreshCw, Reply, Send, Trash2, X } from "lucide-react";
...
import { fileToUpload, formatStamp, hexToInt, isTextLike } from "@/lib/discord/format";
...
function Composer({
  ...
  const [pendingFiles, setPendingFiles] = useState<
    { filename: string; contentType: string; dataBase64: string; previewUrl: string }[]
  >([]);
  ...
  async function send() {
    ...
    try {
      ...
      const files = pendingFiles.map(({ filename, contentType, dataBase64 }) => ({
        filename,
        contentType,
        dataBase64,
      }));
      await sendMessage(channelId, payload, embeds, {
  ...
  async function onPickImages(list: FileList | null) {
    ...
    for (const file of Array.from(list)) {
      ...
      try {
        ...
        next.push({ ...up, previewUrl: URL.createObjectURL(file) });
      } catch (err) {
        toast.error(err instanceof Error ? err.message : "Could not read image");
      }
    }
    setPendingFiles(next.slice(0, 10));
  }
    ...
    <div className="shrink-0 border-t border-border p-3 sm:p-4">
      ...
      <div className="relative rounded-lg border border-border bg-card p-2 sm:p-3">
        ...
        <Textarea
          ...
          placeholder={replyTo ? "Write a reply…" : "Message as the bot — Image to attach"}
        ...
        {content.trim() && preview.includes("<@") ? (
          <p className="mt-1 px-1 font-mono text-[10px] text-muted-foreground">Send payload: {preview}</p>
        ) : null}
        {mentionOpen ? (
              ...
              <div key={i} className="relative">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setPendingFiles((cur) => cur.filter((_, j) => j !== i));
                  }}
                >
                  <X className="size-3.5" />
                </button>
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <label className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-md border border-border bg-background px-3 text-sm hover:bg-secondary">
              <input
                type="file"
